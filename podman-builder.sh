...
CRED_PATH="${XDG_RUNTIME_DIR}/containers/auth.json"

if [ -f "${CRED_PATH}" ]; then
    echo "auth for local user: \`${USER}\` found!"
else
    echo "no podman login found!"
    echo "re-run the script after logging in with \`podman login [registry]\`"
    echo "exiting..."
    exit 1
fi
...
function push_prod(){
    podman push "${BASE_TAG_NAME}:$VERSION_TAG"
    podman push "${BASE_TAG_NAME}:latest"
}
